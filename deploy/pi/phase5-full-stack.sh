...
#   - rsync + ssh to $PI (default taktflow-pi@192.168.0.197)
...
#   2. rsync binary + opensovd-pi.toml to /opt/taktflow/sovd-main/
...
log "[3/6] rsync opensovd-pi.toml -> $PI:$REMOTE_SOVD_DIR/opensovd.toml"
rsync -az "$DEPLOY_DIR/opensovd-pi.toml" "$PI:$REMOTE_SOVD_DIR/opensovd.toml"
...
sleep 2
if curl -fsS --max-time 5 "http://192.168.0.197:21002/sovd/v1/components" >/dev/null; then
    log "sovd-main answering GET /sovd/v1/components on 192.168.0.197:21002 — D1 green"
else
    err "sovd-main is NOT answering on 192.168.0.197:21002"
